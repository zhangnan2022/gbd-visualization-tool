...
with tabs[0]:
    ...
    if uploaded_file is not None:
        ...
        selected_age = st.selectbox("请选择 age", ["请选择"] + age_options)

        # 根据所选年龄组动态生成 metric 选项
        if selected_age != "请选择":
            filtered_metric_options = df[df['age'] == selected_age]['metric'].dropna().unique().tolist()
        else:
            filtered_metric_options = df['metric'].dropna().unique().tolist()

        selected_metric = st.selectbox("请选择 metric", ["请选择"] + filtered_metric_options)

        selected_sex = st.multiselect("请选择 sex", sex_options) if group_by != "sex" else []
        selected_age_group = st.multiselect("多选 age", age_options) if group_by != "age" else []
        ...
        if selected_measure != "请选择" and selected_location != "请选择" and selected_cause != "请选择" and selected_metric != "请选择":
            ...
            if group_by != "age" and selected_age_group:
                chart_df = chart_df[chart_df['age'].isin(selected_age_group)]
            if selected_age != "请选择" and group_by != "age":
                chart_df = chart_df[chart_df['age'] == selected_age]
...
with tabs[1]:
    ...
    if table_file is not None:
        ...
        selected_metric = st.selectbox("请选择 metric", available_metrics)
        ...
        def get_row(location):
            ...
            for year in year_range:
                d_number = df[(df['location'] == location) & (df['year'] == year) & (df['cause'] == selected_cause) &
                              (df['metric'] == selected_metric) & (df['measure'] == selected_measure) &
                              (df['age'] == selected_age) & (df['sex'] == selected_sex)]

                if not d_number.empty:
                    row[f"Val_{year}"] = format_val(d_number['val'].sum(), d_number['lower'].sum(), d_number['upper'].sum(), 2 if selected_metric != "Number" else 0)
                else:
                    row[f"Val_{year}"] = "NA"
            ...
            if not d_1990.empty and not d_2021.empty:
                ...
                row['Change (%)'] = f"{round(change, 2)}%"
            else:
                row['Change (%)'] = "NA"

            d_all = df[(df['location'] == location) & (df['cause'] == selected_cause) &
                       (df['metric'] == selected_metric) & (df['measure'] == selected_measure) &
                       (df['age'] == selected_age) & (df['sex'] == selected_sex)]
            ...
            if d_all.shape[0] >= 2:
                try:
                    d_all = d_all[d_all['val'] > 0]
                    d_all['y'] = np.log(d_all['val'])
                    slope, intercept, r_value, p_value, std_err = stats.linregress(d_all['year'], d_all['y'])
                    ...
                    pass
            row['EAPC (95% CI)'] = eapc_ci

            return row

        if st.button("生成 Table 1"):
            order = ['Global', 'High SDI', 'High-middle SDI', 'Middle SDI', 'Low-middle SDI', 'Low SDI']
            all_locs = order + [loc for loc in df['location'].unique() if loc not in order]
            summary_df = []
